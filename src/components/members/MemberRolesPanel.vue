  ...
  <Card class="min-h-0" size="sm">
      ...
      <template v-else-if="selectedMember">
        <div class="flex items-center gap-3 rounded-lg border bg-muted/30 p-3">
          ...
          <div class="min-w-0 flex-1">
            ...
            <p class="truncate text-xs text-muted-foreground">{{ selectedMember.user?.email ?? selectedMember.userId }}</p>
          </div>
        </div>
    ...
    <CardFooter class="flex flex-wrap items-center justify-between gap-2 border-t px-3 py-3">
      <div class="text-xs text-muted-foreground">
        {{ canUpdate ? "Changes are applied only after saving." : "You need update permission to edit member roles." }}
      </div>
      <div class="flex flex-wrap justify-end gap-2">
        <Button
          variant="outline"
